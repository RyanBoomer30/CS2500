...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname |CS2500_HW7 copy|) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
(define (build-transaction serial priv pub amt)
  (make-transaction
   serial
   (make-signature (string-append pub ":" (number->string amt)) priv)
   (secret->public priv)
   pub
   amt))

(check-expect (build-transaction 0 ALICE-PRIVATE-KEY BOB-PUBLIC-KEY 10)
              (make-transaction
               0
               "kPvx/cgkOyztaqkSMAbOuN9gGPIrCx8xqvwDaJrT4TPvuLlxxXvbpz/iidSzOrZsZ484INk3vEOwjYaLlt57ng=="
               ALICE-PUBLIC-KEY
               BOB-PUBLIC-KEY
               10))

(check-expect (build-transaction 1 BOB-PRIVATE-KEY ALICE-PUBLIC-KEY 10)
              (make-transaction
               1
               "V98VAVk5RAjLFSHtBwPoTDq1/hrpN1KcFiBq4kZ4fKAtUy/pQWStqaJD5OJ636JFefb7MdrhmZGcKT+enVhPcQ=="
               BOB-PUBLIC-KEY
               ALICE-PUBLIC-KEY      
               10))
...
(check-expect (transaction->string EX-TRANSACTION-0)
              "transaction:Gz1+7eGYml1+nHrpaA0EIwqTgg3sTKfa06Kqp7ON5+Dh9KrF19vhTeLT+iOnnyhzh5ag6fiP5Ez2RILE3yzNoA==:AAAAB3NzaC1yc2EAAAADAQABAAAAQQDKzzn3n3b6MDsAKZcWPzS5CRcpsnnYpL8L7MobUPfAmsDV3isnYPlmA/WM0FtbSGOU2tW6mYZOYaVhjOHrrCsZ:AAAAB3NzaC1yc2EAAAADAQABAAAAQQCsuMztsNR31zDR0gmCroXg/uWZAJR7m3j2DTlb4t4APwENUtB3ERZk5Mxc/0gqftHGd8BK8x4Sl5r4CK4y23WD,100")

(check-expect (transaction->string (build-transaction 0 ALICE-PRIVATE-KEY BOB-PUBLIC-KEY 10))
              "transaction:kPvx/cgkOyztaqkSMAbOuN9gGPIrCx8xqvwDaJrT4TPvuLlxxXvbpz/iidSzOrZsZ484INk3vEOwjYaLlt57ng==:AAAAB3NzaC1yc2EAAAADAQABAAAAQQDKzzn3n3b6MDsAKZcWPzS5CRcpsnnYpL8L7MobUPfAmsDV3isnYPlmA/WM0FtbSGOU2tW6mYZOYaVhjOHrrCsZ:AAAAB3NzaC1yc2EAAAADAQABAAAAQQCsuMztsNR31zDR0gmCroXg/uWZAJR7m3j2DTlb4t4APwENUtB3ERZk5Mxc/0gqftHGd8BK8x4Sl5r4CK4y23WD,10")
...
(define (block-digest prev-digest block)
  (digest (string-append (number->string prev-digest)
                         (foldr (lambda (x y) (string-append (transaction->string x) y))
                                "" (block-transactions block)) (number->string (block-nonce block)))))
...
(define EX-BLOCK-0 (make-block '() 8631727707325622792404128232286945630015639849891523695238049493932286431978
                               ALICE-PUBLIC-KEY))

;; Example block 1 that links to the previous the genesis block
(define EX-BLOCK-1 (mine-block 0 ALICE-PUBLIC-KEY
                               (list
                                (build-transaction 0 ALICE-PRIVATE-KEY BOB-PUBLIC-KEY 10)
                                ...
                                (build-transaction 4 ALICE-PRIVATE-KEY BOB-PUBLIC-KEY 10)) 1000000))
...
(define (map-transactions f bc)
  (foldr (lambda (x y)
           (append (map f (block-transactions x)) y))
         empty
         bc))
...
(define (no-duplicate-transactions? bc)
  (local
    ([define serial-list
       (sort (map-transactions transaction-serial bc) <)])
...
(check-expect (reward ALICE-PUBLIC-KEY (make-hash '())) (make-hash (list (list ALICE-PUBLIC-KEY 100))))
...
(define (update-ledger/transaction tr l)
  (cond
    [(or
      (not (hash-has-key? l (transaction-sender-key tr)))
...
(check-expect (update-ledger/transaction EX-TRANSACTION-0 (make-hash (list (list ALICE-PUBLIC-KEY 1000))))
              (make-hash (list
                          (list ALICE-PUBLIC-KEY 900)
                          (list BOB-PUBLIC-KEY 100))))
...
(define (update-ledger/blockchain bc ledge)
  (cond
    ...
    [else
     (local
       ([define prev (update-ledger/blockchain (rest bc) ledge)])
...
(define (valid-blockchain? bc)
  (and (blocks-sized-ok? bc) (no-duplicate-transactions? bc) (all-signatures-ok? bc) (valid-digests? bc)

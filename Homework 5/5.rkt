...
(define (table-of-contents los)
  (local [;; not-contain-content : [List-of String] -> Boolean
    ...
    (cond
      [(empty? los) '()]
      [(cons? los)
       (map substring-title (take-while not-empty-string (rest (drop-while not-contain-content los))))
       ])))
...
(define (extract-works toc lines)
  (local [
          ...
          (define reverse-toc
            (reverse (table-of-contents lines)))
          ...
          (define list-of-text
            (take-while before-end (drop-while not-after-toc lines)))
    ...
    (cond
      [(empty? toc) '()]
      [(cons? toc)
       (map create-work (group-by end-of-work (filter not-empty list-of-text)))
       ])))

;; (extract-works (table-of-contents (read-lines "pg100.txt")) (read-lines "pg100.txt"))
...
(define (word-frequency top-words word-list)
  (local [;; top-words-filter : String -> Boolean
          ...
          (define (filter-topword lst top-word-lst)
            (cond
              ...
              [(cons? lst)
               (cons
               (if (member? (first top-word-lst) (map (lambda (x) (if (empty? x)
                                                                      x
                                                                      (first x))) lst))
    ...
    (cond
      [(empty? word-list) '()]
      [(cons? word-list)
       (map length (filter-topword (group-by string=? (filter top-words-filter (sort word-list string<=?)))
                                   top-words))])))

(check-expect (word-frequency (top-n-words 3 (list "a" "a" "b" "c" "b" "c" "c"))
                              (list "a" "a" "b" "c" "b" "c" "c")) (list 3 2 2))
(check-expect (word-frequency (top-n-words 1 (list "a" "a" "b" "c" "b" "c" "c"))
                              (list "a" "a" "b" "c" "b" "c" "c")) (list 3))
(check-expect (word-frequency (top-n-words 0 (list "a" "a" "b" "c" "b" "c" "c"))
                              (list "a" "a" "b" "c" "b" "c" "c")) '())

;; my-map-with-extra : (Number X -> Y) [List-of X] -> [List-of Y]
(define (my-map-with-extra the-number the-function los)
  (cond
    [(empty? los) '()]
    [(cons? los)
     (cons (the-function the-number (first los))
           (my-map-with-extra the-number the-function (rest los)))]))
...
(define (document-distance top-words word-list-1 word-list-2)
  (local [(define (distance x y)
            (expt (- x y) 2)
            )]
    (sqrt (foldr + 0 (map distance
...
(check-expect (document-distance (top-n-words 2 (list "a" "a" "b" "c" "b" "c" "c"))
                                 (list "a" "a" "b" "c" "b" "c" "c")
                                 (list "a" "a" "b" "c" "b" "c" "c")) 0)
(check-expect (document-distance (top-n-words 2 (list "a" "a" "b" "c" "b" "c" "c"))
                                 (list "a" "a" "b" "c" "b" "c" "c")
                                 (list "a" "b" "b" "b" "b" "a" "c")) 2)
(check-expect (document-distance (top-n-words 2 (list "a" "a" "b" "c" "b" "c" "c"))
                                 (list "")
                                 (list "")) 0)
...
(define (similar-works-template w)
  (... (similar-works-work w) ...
       (similar-works-distance w) ...
       ))
...
(define (test-distance n firstArticle allArticles)
  (cond
    ...
    [(cons? allArticles)
      (cons
      (make-similar-works  
       (make-work (string-append (work-title (first allArticles)) ".txt") (read-lines
                                                                            (string-append
                                                                             (work-title (first allArticles)) ".txt")))
...
(map similar-works-work (sort (test-distance 10 "KING JOHN" (extract-works(table-of-contents
                              (read-lines "pg100.txt"))
                             (read-lines "pg100.txt"))) (lambda (article1 article2)
                                                          (< (similar-works-distance article1)
                                                             (similar-works-distance article2))
                                                          )))

#| (map document-distance/file (top-n-words/file 4 (first (extract-works-to-files (read-lines "pg100.txt"))))
     (first (extract-works-to-files (read-lines "pg100.txt")))
     (rest (extract-works-to-files (read-lines "pg100.txt")))) |#